...
export default function LandingPage() {
  return (
    <div className="min-h-screen bg-background text-foreground">
      {/* ── Header/Nav ── */}
      <header className="fixed top-0 inset-x-0 z-50 bg-background/80 backdrop-blur-lg border-b border-border/40">
        <div className="max-w-7xl mx-auto flex items-center justify-between px-4 sm:px-6 h-16">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link to="/cadastro">
              <Button size="sm" className="shadow-md shadow-primary/20">Criar conta grátis</Button>
            </Link>
          </div>
        </div>
      </header>
      ...
      <section className="pt-32 pb-20 px-4 sm:px-6 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center relative">
          <div className="inline-flex items-center gap-2 rounded-full bg-primary/10 px-4 py-1.5 text-xs font-medium text-primary mb-6">
            <Zap className="h-3.5 w-3.5" /> Novo: Gerador de questões com IA
          </div>
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-extrabold tracking-tight font-display leading-[1.1] mb-6">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link to="/cadastro">
              <Button size="lg" className="text-base px-8 shadow-xl shadow-primary/25 gap-2">
          ...
          <p className="text-xs text-muted-foreground mt-4">Sem cartão de crédito · Setup em 2 minutos</p>
        </div>
      </section>
      ...
      <section id="funcionalidades" className="py-20 px-4 sm:px-6 bg-muted/30">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {features.map((f) => (
              <Card key={f.title} className="group hover:shadow-lg hover:shadow-primary/5 transition-all duration-300 border-border/50 bg-card">
      ...
      <section id="depoimentos" className="py-20 px-4 sm:px-6">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {testimonials.map((t) => (
              <Card key={t.name} className="border-border/50 bg-card relative overflow-hidden">
                <CardContent className="p-6">
      ...
      <section id="planos" className="py-20 px-4 sm:px-6 bg-muted/30">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {plans.map((p) => (
              <Card key={p.name} className={`border-border/50 bg-card relative ${p.highlight ? "ring-2 ring-primary shadow-xl shadow-primary/10" : ""}`}>
                {p.highlight && (
      ...
      <section className="py-20 px-4 sm:px-6">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <Link to="/cadastro">
            <Button size="lg" className="text-base px-10 shadow-xl shadow-primary/25 gap-2">
